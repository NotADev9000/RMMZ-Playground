...
Char_Type_Base.prototype.updateMove = function(character) {
    if (character._x < character._realX) {
        character._realX = Math.max(character._realX - character.distancePerFrame(), character._x);
    }
    if (character._x > character._realX) {
        character._realX = Math.min(character._realX + character.distancePerFrame(), character._x);
    }
    if (character._y < character._realY) {
        character._realY = Math.max(character._realY - character.distancePerFrame(), character._y);
    }
    if (character._y > character._realY) {
        character._realY = Math.min(character._realY + character.distancePerFrame(), character._y);
    }
